...
<title>AI Store — Universe of AI</title>
<link rel="manifest" href="manifest.json">
<meta name="theme-color" content="#7c5cff">
<meta name="apple-mobile-web-app-capable" content="yes">
<meta name="apple-mobile-web-app-status-bar-style" content="black-translucent">
<meta name="apple-mobile-web-app-title" content="AI Store">
<meta name="description" content="100+ Best AI Tools in One Place — Free & Paid">
...
<style>
:root{--bg:#05050f;--card:#0d0d1f;--card2:#111128;--border:rgba(255,255,255,0.07);--accent:#7c5cff;--accent2:#ff5cad;--accent3:#5cffb8;--text:#e8e8f0;--muted:#6b6b8a}
*{margin:0;padding:0;box-sizing:border-box}
html{font-size:16px}
body{font-family:'Space Grotesk',sans-serif;background:var(--bg);color:var(--text);min-height:100vh;overflow-x:hidden;width:100%;max-width:100%}
body::before{content:'';position:fixed;inset:0;background-image:radial-gradient(1px 1px at 10% 20%,rgba(124,92,255,.5) 0%,transparent 100%),radial-gradient(1px 1px at 80% 10%,rgba(255,92,173,.4) 0%,transparent 100%),radial-gradient(1px 1px at 50% 50%,rgba(92,255,184,.3) 0%,transparent 100%);background-size:400px 400px;pointer-events:none;z-index:0}
.orb1{position:fixed;width:600px;height:600px;border-radius:50%;background:radial-gradient(circle,rgba(124,92,255,.12) 0%,transparent 70%);top:-200px;left:-200px;pointer-events:none;z-index:0;animation:float1 8s ease-in-out infinite}
.orb2{position:fixed;width:500px;height:500px;border-radius:50%;background:radial-gradient(circle,rgba(255,92,173,.1) 0%,transparent 70%);bottom:-150px;right:-150px;pointer-events:none;z-index:0;animation:float2 10s ease-in-out infinite}
@keyframes float1{0%,100%{transform:translate(0,0)}50%{transform:translate(30px,30px)}}
@keyframes float2{0%,100%{transform:translate(0,0)}50%{transform:translate(-20px,-20px)}}
header{position:sticky;top:0;z-index:10;padding:18px 40px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid var(--border);backdrop-filter:blur(20px);background:rgba(5,5,15,.85)}
...
.header-badge{background:linear-gradient(135deg,var(--accent),var(--accent2));color:white;font-size:11px;font-weight:600;padding:4px 12px;border-radius:20px;letter-spacing:.5px;text-transform:uppercase}
.hero{position:relative;z-index:5;text-align:center;padding:40px 20px 30px}
.hero-tag{display:none}
.hero h1{font-family:'Syne',sans-serif;font-size:clamp(22px,4vw,52px);font-weight:800;line-height:1.15;letter-spacing:-1px;margin-bottom:12px}
.hero h1 .g1{background:linear-gradient(135deg,#fff 0%,rgba(255,255,255,.7) 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.hero h1 .g2{background:linear-gradient(135deg,var(--accent) 0%,var(--accent2) 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
...
.search-wrap input{width:100%;background:var(--card);border:1px solid var(--border);border-radius:50px;padding:14px 50px 14px 22px;color:var(--text);font-size:15px;font-family:'Space Grotesk',sans-serif;outline:none;transition:border-color .2s}
.search-wrap input:focus{border-color:rgba(124,92,255,.5)}
.search-wrap input::placeholder{color:var(--muted)}
.search-wrap .si{position:absolute;right:18px;top:50%;transform:translateY(-50%);color:var(--muted);font-size:18px}
.stats{display:flex;justify-content:center;gap:40px;margin:20px 0 40px;position:relative;z-index:5}
.stat{text-align:center}
.stat-num{font-family:'Syne',sans-serif;font-size:28px;font-weight:800;background:linear-gradient(135deg,var(--accent),var(--accent2));-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.stat-label{color:var(--muted);font-size:12px;margin-top:2px;text-transform:uppercase;letter-spacing:.5px}
.categories{position:relative;z-index:5;display:flex;gap:8px;justify-content:center;flex-wrap:wrap;padding:0 40px 36px}
.cat-btn{background:var(--card);border:1px solid var(--border);color:var(--muted);padding:8px 16px;border-radius:30px;font-size:12.5px;font-weight:500;cursor:pointer;transition:all .2s;font-family:'Space Grotesk',sans-serif}
.cat-btn:hover,.cat-btn.active{background:var(--accent);border-color:var(--accent);color:white}
.section{position:relative;z-index:5;padding:0 40px 80px;max-width:1300px;margin:0 auto}
.section-title{font-family:'Syne',sans-serif;font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:2px;color:var(--muted);margin-bottom:20px;display:flex;align-items:center;gap:10px}
.section-title::after{content:'';flex:1;height:1px;background:var(--border)}
.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(265px,1fr));gap:14px}
.card{background:var(--card);border:1px solid var(--border);border-radius:18px;padding:20px;cursor:pointer;transition:all .25s cubic-bezier(.4,0,.2,1);animation:fadeUp .4s ease both}
.card:hover{transform:translateY(-4px);border-color:rgba(124,92,255,.3);background:var(--card2);box-shadow:0 20px 50px rgba(0,0,0,.5),0 0 0 1px rgba(124,92,255,.12)}
@keyframes fadeUp{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}
.card-header{display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:12px}
.card-icon{width:46px;height:46px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:22px;background:rgba(255,255,255,.04);flex-shrink:0}
.card-badges{display:flex;gap:4px;flex-wrap:wrap;justify-content:flex-end}
.badge{font-size:10px;font-weight:600;padding:3px 7px;border-radius:20px;text-transform:uppercase;letter-spacing:.5px}
.badge-free{background:rgba(92,255,184,.1);color:var(--accent3);border:1px solid rgba(92,255,184,.2)}
.badge-paid{background:rgba(255,200,92,.1);color:#ffc85c;border:1px solid rgba(255,200,92,.2)}
.badge-new{background:rgba(124,92,255,.1);color:var(--accent);border:1px solid rgba(124,92,255,.2)}
.badge-hot{background:rgba(255,92,92,.1);color:#ff5c5c;border:1px solid rgba(255,92,92,.2)}
.card-name{font-family:'Syne',sans-serif;font-size:16px;font-weight:700;margin-bottom:3px}
.card-category{font-size:11px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px}
.card-desc{font-size:12.5px;color:var(--muted);line-height:1.55;margin-bottom:14px}
.card-footer{display:flex;align-items:center;justify-content:space-between}
.rating{font-size:12px;color:#ffc85c}
.open-btn{background:linear-gradient(135deg,var(--accent),var(--accent2));color:white;border:none;padding:6px 14px;border-radius:20px;font-size:12px;font-weight:600;cursor:pointer;font-family:'Space Grotesk',sans-serif;transition:opacity .2s;text-decoration:none;display:inline-block}
.open-btn:hover{opacity:.85}
.featured-card{background:linear-gradient(135deg,rgba(124,92,255,.15),rgba(255,92,173,.1));border:1px solid rgba(124,92,255,.2);border-radius:22px;padding:28px;display:grid;grid-template-columns:auto 1fr auto;gap:20px;align-items:center;margin-bottom:36px;position:relative;overflow:hidden;transition:all .25s;cursor:pointer}
.featured-card:hover{transform:translateY(-3px);box-shadow:0 30px 80px rgba(124,92,255,.2)}
.featured-card::after{content:'FEATURED';position:absolute;top:18px;right:18px;background:linear-gradient(135deg,var(--accent),var(--accent2));color:white;font-size:10px;font-weight:700;padding:4px 10px;border-radius:20px;letter-spacing:1px}
.featured-icon{width:70px;height:70px;border-radius:20px;display:flex;align-items:center;justify-content:center;font-size:36px;background:rgba(255,255,255,.05)}
.featured-name{font-family:'Syne',sans-serif;font-size:22px;font-weight:800;margin-bottom:5px}
.featured-desc{color:var(--muted);font-size:13px;line-height:1.5;max-width:380px}
#noResults{display:none;text-align:center;padding:60px;color:var(--muted)}
.install-banner{position:fixed;bottom:20px;left:50%;transform:translateX(-50%);background:linear-gradient(135deg,var(--accent),var(--accent2));color:white;padding:12px 24px;border-radius:50px;font-size:14px;font-weight:600;cursor:pointer;z-index:100;box-shadow:0 10px 40px rgba(124,92,255,.4);display:none;align-items:center;gap:10px;white-space:nowrap}
footer{position:relative;z-index:5;text-align:center;padding:36px;border-top:1px solid var(--border);color:var(--muted);font-size:13px}
@media(max-width:768px){
  ...
  .stats{gap:12px;margin:16px 0 28px}
  .stat-num{font-size:20px}
  .stat-label{font-size:10px}
  .section{padding:0 12px 50px}
  .categories{padding:0 12px 20px;gap:6px}
  ...
  .search-wrap input{font-size:14px;padding:11px 40px 11px 16px}
}
</style>
</head>
<body>
...
<header>
  <div class="logo">AI<span>.</span>Store <em>✦</em></div>
  <div class="header-badge">🚀 <span id="headerCount">100</span>+ AI Tools</div>
</header>
<div class="hero">
  <div class="hero-tag">✦ The Universe of AI</div>
  <h1><span class="g1">Discover Every</span><br><span class="g2">AI Tool Ever</span></h1>
  <p>100+ best AI tools in one place — both free and paid!</p>
  <div class="search-wrap">
    <input type="text" id="searchInput" placeholder="Search any AI tool..." oninput="filterCards()">
    <span class="si">🔍</span>
  </div>
</div>
<div class="stats">
  <div class="stat"><div class="stat-num" id="totalCount">100+</div><div class="stat-label">AI Tools</div></div>
  <div class="stat"><div class="stat-num" id="catCount">9</div><div class="stat-label">Categories</div></div>
  <div class="stat"><div class="stat-num">∞</div><div class="stat-label">Possibilities</div></div>
</div>
<div class="categories" id="catButtons"></div>
<div class="section">
  <div id="featured-section">
    <div class="section-title">⭐ Featured Pick</div>
    <div class="featured-card" onclick="window.open('https://claude.ai','_blank')">
      <div class="featured-icon">🤖</div>
      <div>
        <div class="featured-name">Claude by Anthropic</div>
        <div style="color:var(--muted);font-size:11px;margin-bottom:6px;text-transform:uppercase;letter-spacing:.5px">Chat AI</div>
        <div class="featured-desc">The smartest and safest AI assistant — coding, writing, analysis, research all in one place!</div>
      </div>
      <a href="https://claude.ai" target="_blank" class="open-btn" onclick="event.stopPropagation()">Open ↗</a>
    </div>
  </div>
  <div class="section-title">🔥 All AI Tools</div>
  <div class="grid" id="aiGrid"></div>
  <div id="noResults"><div style="font-size:48px;margin-bottom:14px">🤖</div><div style="font-size:16px;margin-bottom:6px">No AI tool found!</div><div>Try searching something else 😊</div></div>
</div>
<div class="install-banner" id="installBanner" onclick="installApp()">📱 Install AI Store App — Free!</div>
<footer>
  Made with ❤️ — AI Store © 2026 &nbsp;|&nbsp; 100+ AI Tools from around the world in one place!<br>
  <small style="opacity:.5;font-size:11px;margin-top:6px;display:block">This site contains affiliate links. We may earn a commission at no extra cost to you.</small>
</footer>
<script>
// ============================================================
// 🛠️ ADD / EDIT / DELETE TOOLS FROM HERE!
// Each tool format:
// { name:"Tool Name", icon:"🤖", cat:"chat", desc:"What it does", badge:"free", link:"https://...", rating:4.5 },
// Categories: "chat" | "image" | "video" | "code" | "voice" | "search" | "productivity" | "writing" | "avatar"
// To add a new tool: Just add one line below!
// ============================================================
const tools=[
  // ===== 💬 CHAT AI =====
...
let cC='all',cS='';
function gCL(c){const x=categories.find(i=>i.id===c);return x?x.label.replace(/^[^\w\s]+\s*/,''):c}
function bC(){
  const w=document.getElementById('catButtons');
  w.innerHTML=categories.map((c,i)=>`<button class="cat-btn${i===0?' active':''}" onclick="fC('${c.id}',this)">${c.label}</button>`).join('');
  document.getElementById('catCount').textContent=categories.length-1;
}
function rG(){
  const g=document.getElementById('aiGrid'),n=document.getElementById('noResults'),f=document.getElementById('featured-section');
  let fl=tools.filter(t=>(cC==='all'||t.cat===cC)&&(t.name.toLowerCase().includes(cS.toLowerCase())||t.desc.toLowerCase().includes(cS.toLowerCase())));
  f.style.display=(cC==='all'&&!cS)?'block':'none';
  document.getElementById('totalCount').textContent=fl.length;
  document.getElementById('headerCount').textContent=tools.length;
  if(!fl.length){g.innerHTML='';n.style.display='block';return;}
  n.style.display='none';
  g.innerHTML=fl.map((t,i)=>`
    <div class="card" style="animation-delay:${Math.min(i,25)*.03}s" onclick="window.open('${t.link}','_blank')">
      <div class="card-header">
        <div class="card-icon">${t.icon}</div>
        <div class="card-badges">
          ${t.badge==='free'?'<span class="badge badge-free">Free</span>':''}
          ${t.badge==='paid'?'<span class="badge badge-paid">Paid</span>':''}
          ${t.badge==='new'?'<span class="badge badge-new">New</span>':''}
          ${t.badge==='hot'?'<span class="badge badge-hot">🔥 Hot</span>':''}
        </div>
      </div>
      <div class="card-name">${t.name}</div>
      <div class="card-category">${gCL(t.cat)}</div>
      <div class="card-desc">${t.desc}</div>
      <div class="card-footer">
        <div class="rating">⭐ ${t.rating}</div>
        <a href="${t.link}" target="_blank" class="open-btn" onclick="event.stopPropagation()">Open ↗</a>
      </div>
    </div>
  `).join('');
}
function fC(c,b){cC=c;document.querySelectorAll('.cat-btn').forEach(x=>x.classList.remove('active'));b.classList.add('active');rG();}
function filterCards(){cS=document.getElementById('searchInput').value;rG();}
let dP;
window.addEventListener('beforeinstallprompt',e=>{e.preventDefault();dP=e;document.getElementById('installBanner').style.display='flex';});
function installApp(){if(dP){dP.prompt();dP.userChoice.then(()=>{dP=null;document.getElementById('installBanner').style.display='none';});}}
document.addEventListener('contextmenu',e=>e.preventDefault());
document.addEventListener('keydown',e=>{if(e.ctrlKey&&(e.key==='u'||e.key==='s'))e.preventDefault();});
bC();rG();
</script>
</body>
</html>
